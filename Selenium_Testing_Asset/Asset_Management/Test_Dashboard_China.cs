...
{
    public class Test_Dashboard_China
    {
        ...
        public void Asset_Dashboard_Process(DataSet ds, string ID, string url, string UserID, string Pwd)
        {
            ...
            //string[] companylist = { "골프존", "네오위즈 에이블 스튜디오" };

            //All_company_total_summary(companylist);
            All_company_total_summary_ds(ds, taskID);
            /* Test용 */
            /*
            bspm.Bsp_Company_list();
            bspm.Bsp_Select_Company("(주)스파이카");
            Test_Test("(주)스파이카");
            */
            //bspm.Bsp_Company_list();
            //bspm.Bsp_Select_Company("Hostway US");
        }
        ...
        public void All_company_total_summary_ds(DataSet companylist, string taskID)
        {
            ...
            for (int i = 0; i < companylist.Tables[0].Rows.Count; i++)
            {
                ...
                dbmanager.ExecuteDataQuery(string.Format("exec prc_Task_Status_Update '{0}'", taskID), "Asset_Management_china");
            }
        ...
        public void total_summary(string companyNum)
        {
            ...
            foreach (IWebElement summarys in resultitem)
            {
                if (summarys.Text.Replace("\r\n", "|").Split('|')[0] == "Total Server")
                {
                    ...
                    dbmanager.ExecuteDataQuery(querystring, "Asset_Management_china");
                }
                ...
                if (title == "SERVER")
                {
                    ...
                    foreach (IWebElement sitem in Serverresource)
                    {
                        ...
                        if (tb_t == "TOTAL")
                        {
                            ...
                            dbmanager.ExecuteDataQuery(total_query, "Asset_Management_china");
                        }
                        else if (tb_t == "AWS")
                        {
                            ReadOnlyCollection<IWebElement> table1 = sitem.FindElements(By.XPath(".//tbody/tr"));
                ...
                else if (title == "DATABASE & STORAGE")
                {
                    ...
                    foreach (IWebElement sitem in Serverresource)
                    {
                        ...
                        if (tb_t == "TOTAL")
                        {
                            ...
                            dbmanager.ExecuteDataQuery(total_qurey_string, "Asset_Management_china");
                        }
                        else if (tb_t == "AWS")
                        {
                            ReadOnlyCollection<IWebElement> table1 = sitem.FindElements(By.XPath(".//tbody/tr"));
        ...
        public void Result_Check(string taskno)
        {
            ...
            foreach (DataTable table in companylist.Tables)
            {
                foreach (DataRow row in table.Rows)
                {
                    ...
                    DataSet tb_summary_data = dbmanager.ExecuteDataQuery(tb_summary_query, "Asset_Management_china");

                    string tb_resource_query = string.Format("select * from [dbo].[TB_Dashboard_Resources] where [company_name] = '{0}'", row["company_name"]);
                    DataSet tb_resource_data = dbmanager.ExecuteDataQuery(tb_resource_query, "Asset_Management_china");

                    string tb_server_query = string.Format("exec pro_Dashboard_Server_Q '{0}'", row["company_name"]);
                    DataSet tb_server_data = dbmanager.ExecuteDataQuery(tb_server_query, "Asset_Management_china");

                    string tb_db_query = string.Format(@"exec pro_Dashboard_Database_Q '{0}'", row["company_name"]);
                    DataSet tb_db_data = dbmanager.ExecuteDataQuery(tb_db_query, "Asset_Management_china");

                    string tb_Network_query = string.Format(@"exec pro_Dashboard_Network_Q '{0}'", row["company_name"]);
                    DataSet tb_Network_data = dbmanager.ExecuteDataQuery(tb_Network_query, "Asset_Management_china");
                    ...
                    dbmanager.ExecuteDataQuery(autocheck_result, "Asset_Management_china");
                }
            }
        }
        ...
        public void Result_Check_Detail(string company_name, DataSet summary, DataSet resource, DataSet server, DataSet db, DataSet network)
        {
            ...
            if (summaryrow != null)
            {
                ...
                DataSet ds = dbmanager.ExecuteDataQuery(Qry, "Asset_Management_china");
            }
            ...
            if (resource.Tables.Count > 0)
            {
                if (resource.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < resource.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (dr["Resource_Name"].ToString() == "SQL Database" || dr["Resource_Name"].ToString() == "Data Warehouse")
                        {
                            sqlserver = sqlserver + Convert.ToInt32(dr["Resource_Cnt"]);
                            int total = Convert.ToInt32(dbrows["azure_db_usage"]);
                            int ondemand_run = Convert.ToInt32(summaryrow["db_ondemand_run"]);
                            if (ondemand_run == total)
