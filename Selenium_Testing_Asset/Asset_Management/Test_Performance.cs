    ...
    public class Test_Performance
    {
        ...
        public void Insert_instance_status(string company_name, string servicetype, string resourcename)
        {
            ...
            string qrystring = string.Format("exec prc_Performance_instance_Status_I '{0}','{1}','{2}',{3},{4},{5},{6}"
                ...
                , valstring[4]
                , valstring[6]);
            dbmanager.ExecuteDataQuery(qrystring, "Asset_Management");
        }
        ...
        public void ResultData(string company_name, string servicetype, string resourcename, string tablename)
        {
            ...
            DataSet ResultChart = dbmanager.ExecuteDataQuery(chartqry, "Asset_Management");
            ...
            string statusqry = string.Format("SELECT * FROM [TB_Performance_Instance_Status] Where company_name = '{0}' and servicetype = '{1}' and Resourcetype = '{2}'"
                , company_name, servicetype, resourcename);
            DataSet ResultStatus = dbmanager.ExecuteDataQuery(statusqry, "Asset_Management");
            ...
            if (resourcename == "EC2" || resourcename == "RDS")
            {
                runcntQ = string.Format(@"SELECT Count(*) AS Cnt
                                          FROM [Asset_Management].[dbo].[TB_Performance_Data_{2}_{1}]
            ...
            DataSet runcntD = dbmanager.ExecuteDataQuery(runcntQ, "Asset_Management");
            ...
            foreach (dynamic item in studentObj.Children().ToList())
            {
                ...
                foreach (dynamic subitem in val.Children().ToList())
                {
                    ...
                    {
                        string qry = string.Format(@"SELECT ROUND({0}(Convert(float,$col)),2) AS 'Val' FROM [Asset_Management].[dbo].[{1}] Where  company_name = '{2}'"
                        ...
                        DataSet rs = dbmanager.ExecuteDataQuery(qry, "Asset_Management");
            ...
            dbmanager.ExecuteDataQuery(insertResult, "Asset_Management");

            dbmanager.ExecuteDataQuery(string.Format("exec prc_Performance_Result_ReportData '{0}'", company_name), "Asset_Management");
