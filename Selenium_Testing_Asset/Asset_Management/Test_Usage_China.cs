...
namespace Selenium_Testing_Asset.Asset_Management
{
    public class Test_Usage_China
    {
        ...
        public void Click_Type(string servicetype, string company_name, string taskno)
        {
            ...
            int islarge = Convert.ToInt32(dbmanager.ExecuteDataQuery(string.Format("prc_Select_large_company '{0}'", company_name), "BSP_Management_china").Tables[0].Rows[0][0]);
        ...
        {
            string str = @"SELECT * FROM [Asset_Management_China].[dbo].[TB_Usage_Report_Data]";
            DataSet ds = dbmanager.ExecuteDataQuery(str, "Asset_Management_China");
            if (ds.Tables[0].Rows.Count > 0)
            {
  ...
  FROM [Asset_Management_China].[dbo].[TB_Usage_Auto_Check_Result]
	where company_name = '{0}' and service_type ='{1}' ", dr["company_name"], servicelist[h]
                        );
                        DataSet dg = dbmanager.ExecuteDataQuery(qry, "Asset_Management_China");
                        if (dg.Tables[0].Rows.Count > 0)
                        {
                            ...
                            dbmanager.ExecuteDataQuery(awsq, "Asset_Management_China");
                        }
                    }
                }
        ...
        public void Result_Check_Usage_1(string tablename, string company_name)
        {
            ...
            DataSet ResultData_Chart = dbmanager.ExecuteDataQuery(chartdata, "Asset_Management_China");
            ...
            string insertqry = string.Format("update [TB_Usage_Chart_Data] set status_colname = '{0}' where company_name = '{1}' and servicetype = '{2}' and resourcename = '{3}'"
            ...
            if (isStatus != "NON")
            {
                ...
                DataSet d1 = dbmanager.ExecuteDataQuery(s1, "Asset_Management_China");
                if (d1.Tables.Count > 0)
                {
                    ...
                    for (int i = 0; i < d1.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (dr1[isStatus] == null || dr1[isStatus].ToString() == "" && dr1[isStatus].ToString() == string.Empty)
                            ...
                            col_status = dr1[isStatus].ToString().ToUpper(); ;
                        grid_total_cnt = grid_total_cnt + Convert.ToInt32(dr1["cnt"]);

                        if (col_status != "Unkowns")
                        {
                        ...
                        foreach (dynamic item in STATUS)
                        {// (string)item.Name
                            if ((string)item.Name == col_status)
                            {
                        ...
                        string chartval = (string)studentObj["STATUS"][col_status];
            ...
            if (isRegions != "NON")
            {
                ...
                if (d1.Tables.Count > 0)
                {
                    ...
                    {
                        DataRow dr1 = d1.Tables[0].Rows[i];
                        string regionstr = dr1[isRegions].ToString().ToUpper();
                        // Chart의 Instance total 값과 , Grid의 Instance total 값을 비교하기 위한 값 저장
                        grid_total_cnt = grid_total_cnt + Convert.ToInt32(dr1["cnt"]);
                        chart_total_cnt = chart_total_cnt + (int)studentObj["REGIONS"][regionstr];

                        // Status Name Check
                        REGIONS = studentObj["REGIONS"].ToArray();
                        foreach (dynamic item in REGIONS)
                        {// (string)item.Name
                            if ((string)item.Name == regionstr)
                            {
                                isStatusNamecheck = "PASS";
                                list_isStatusNamecheck.Add(isStatusNamecheck);
                            }
                        ...
                        // instance_count_check : 각 Status 별 항목 비교 
                        string chartval = (string)studentObj["REGIONS"][regionstr];
                        if (chartval == dr1["cnt"].ToString())
                            isPass = "PASS";
                        else
                            isPass = "FAIL";
            ...
            if (isTypes != "NON")
            {
                ...
                if (d1.Tables.Count > 0)
                {
                    ...
                    for (int i = 0; i < d1.Tables[0].Rows.Count; i++)
                    {
                        ...
                        {
                            string istypesstring = dr1[isTypes].ToString().ToUpper();
                            grid_total_cnt = grid_total_cnt + Convert.ToInt32(dr1["cnt"]);
                            chart_total_cnt = chart_total_cnt + (int)studentObj["TYPES"][istypesstring];

                            // Status Name Check
                            TYPES = studentObj["TYPES"].ToArray();
                            foreach (dynamic item in TYPES)
                            {// (string)item.Name
                                if ((string)item.Name == istypesstring)
                                {
                                    isStatusNamecheck = "PASS";
                                    list_isStatusNamecheck.Add(isStatusNamecheck);
                                }
                            ...
                            // instance_count_check : 각 Status 별 항목 비교 
                            string chartval = (string)studentObj["TYPES"][istypesstring];
                            if (chartval == dr1["cnt"].ToString())
                                isPass = "PASS";
                            else
                                isPass = "FAIL";
        ...
        public void Result_Update(string colname, string colval, string company_name, string servicetype, string resourcename)
        {
            ...
            dbmanager.ExecuteDataQuery(updateq, "Asset_Management_China");
        }
        ...
        public void Insert_Aws_GridData_I(string Type, string company_name, string servicetype, int rowcnt)
        {
            ...
            //foreach (IWebElement aws in grid_aws)
            {
                ...
                for (int i = 0; i < healerlist.Length; i++)
                {
                    if (i == 0)
                    {
                        Createtablequery = Createtablequery + string.Format(" {0} INT NULL", healerlist[i].Replace(" ", "_").Replace("(", "").Replace(")", "").Replace("Default", "Def").Replace("-", "_").Replace("Desc", "Desendant").Replace("Desendantription", "Description"));
                        col = col + string.Format("{0}", healerlist[i].Replace(" ", "_").Replace("(", "").Replace(")", "").Replace("Default", "Def").Replace("-", "_").Replace("Desc", "Desendant").Replace("Desendantription", "Description"));
                    }
                    else
                    {
                        Createtablequery = Createtablequery + string.Format(", {0} NVARCHAR(300) NULL", healerlist[i].Replace(" ", "_").Replace("(", "").Replace(")", "").Replace("Default", "Def").Replace("-", "_").Replace("Desc", "Desendant").Replace("Desendantription", "Description"));
                        col = col + string.Format(",{0}", healerlist[i].Replace(" ", "_").Replace("(", "").Replace(")", "").Replace("Default", "Def").Replace("-", "_").Replace("Desc", "Desendant").Replace("Desendantription", "Description"));
                    }
                    Aws_Table.Columns.Add(healerlist[i].Replace(" ", "_").Replace("(", "").Replace(")", "").Replace("Default", "Def").Replace("-", "_").Replace("Desc", "Desendant").Replace("Desendantription", "Description"));
                }
                ...
                if (rowcnt > 40)
                {
                    ...
                    if (tablename == "TB_Usage_Data_azu_VirtualNetworkSubnets")
                    {
                        string unkownstr = string.Format("update TB_Usage_Data_azu_VirtualNetworkSubnets Set Status = 'Unkown' where company_name = '{0}' and Status = ''", company_name);
                        dbmanager.ExecuteDataQuery(unkownstr, "Asset_Management_China");
                    }
                }
                else
                {
                ...
                dbmanager.ExecuteDataQuery(jh, "Asset_Management_China");
                //hahaha
        ...
        public void Insert_Aws_Usage_Chart_json(string company_name, string servicetype, string resourcename)
        {
            ...
            dbmanager.ExecuteDataQuery(qry.Replace("$Param", str), "Asset_Management_China");
        }
                    ...
                    for (int i = 0; i < values.Length; i++)
                    {
                        ...
                        if (i > 1)
                        {
                            ...
                            else
                            {
                                string value_ = values[i].Replace(" EA", "");
                                text = text + string.Format(",{0}", value_);
                                dbmanager.ExecuteDataQuery(qry.Replace("$Param", str + text), "Asset_Management_China");
                                text = string.Empty;
                            }
                        }
